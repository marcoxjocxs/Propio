...
export class RealtimeComponent implements OnInit {
  ...
  constructor(private _realtime: AngularFireDatabase,
    private zone: NgZone, private _storage: AngularFireStorage) {
  ...
  onSubmit(miimg) {
    ...
    //1. crear un Id A PARTIR DE LA REFERENCIA al nodo Usuario
    ...
    tarea.then(() => {
      //la imagen ya se subio con el nombre de la key generada
      //Ahora obtendremos la url de descarga de la imagen
      this._storage.ref(key).getDownloadURL().subscribe((url_img)=>{
